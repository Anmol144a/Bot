...
export default async function setupTower(client, db, createCanvas, loadImage, AttachmentBuilderUnused) {
  ...
  client.on('messageCreate', async (msg) => {
    try {
      ...
      const userRow = await dbGet('SELECT * FROM users WHERE id = ?', [msg.author.id]);
      if (!userRow || userRow.balance < bet) return msg.reply('Insufficient balance');

      // deduct bet immediately
      await dbRun('UPDATE users SET balance = balance - ? WHERE id = ?', [bet, msg.author.id]);
      ...
      const buttons = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`tower_diff_${gameId.lastID}_easy`).setLabel('Easy').setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId(`tower_diff_${gameId.lastID}_medium`).setLabel('Medium').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId(`tower_diff_${gameId.lastID}_hard`).setLabel('Hard').setStyle(ButtonStyle.Danger)
      );
  ...
  client.on('interactionCreate', async (i) => {
    ...
    try {
      ...
      if (id.startsWith('tower_diff_')) {
        ...
        const game = await dbGet('SELECT * FROM tower_games WHERE id = ?', [gameId]);
        if (!game) {
        ...
        await dbRun('UPDATE tower_games SET mode = ?, tiles = ?, status = ? WHERE id = ?', [mode, JSON.stringify(tower), 'active', gameId]);
      ...
      if (id.startsWith('tower_pick_')) {
        ...
        await dbRun('UPDATE tower_games SET revealed_rows = ?, current_row = ?, current_multiplier = ?, status = ? WHERE id = ?', [JSON.stringify(revealedRows), row + 1, newMult, status, gameId]);

        const buf = await drawTower(i.user, game.bet_amount, game.mode, tower, revealedRows, status, row + 1, newMult);
        const attachment = new AttachmentBuilder(buf, { name: 'tower.png' });

        if (status === 'lost') {
          await i.message.edit({ content: `💥 <@${i.user.id}> Game over! You hit a dragon.`, files: [attachment], components: [] });
        } else {
          const nextRowButtons = new ActionRowBuilder();
          if (row < 8) {
            for (let c = 0; c < 4; c++) {
              nextRowButtons.addComponents(new ButtonBuilder().setCustomId(`tower_pick_${gameId}_${row + 1}_${c}`).setLabel(`Tile ${c+1}`).setStyle(ButtonStyle.Secondary));
            }
          }
          const cashBtn = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`tower_cash_${gameId}`).setLabel(`Cashout (${newMult.toFixed(2)}x)`).setStyle(ButtonStyle.Success));
          
          const components = row < 8 ? [nextRowButtons, cashBtn] : [cashBtn];
          await i.message.edit({ content: `✅ <@${i.user.id}> Safe! Current multiplier: ${newMult.toFixed(2)}x`, files: [attachment], components });
        }
        return;
      }
      ...
      if (id.startsWith('tower_cash_')) {
        await i.deferUpdate();
        const gameId = Number(id.split('_')[2]);
        ...
        if (game.status !== 'active') {
          await i.followUp({ content: 'Game already ended', ephemeral: true });
          return;
        }

        const winAmount = Math.floor(game.bet_amount * game.current_multiplier);
        await dbRun('UPDATE users SET balance = balance + ? WHERE id = ?', [winAmount, game.user_id]);
        await dbRun('UPDATE tower_games SET status = ? WHERE id = ?', ['cashed_out', gameId]);
        ...
        await i.message.edit({ 
          content: `💰 <@${i.user.id}> Cashed out! Won ${winAmount} points (${game.current_multiplier.toFixed(2)}x multiplier)`, 
          files: [attachment], 
          components: [] 
        });
        return;
      ...
      try {
        await i.followUp({ content: 'An error occurred during the interaction.', ephemeral: true });
      } catch (e) {
